...
export default function PricingSection() {
  ...
  if (loading) return <div className="py-24 text-center"><Loader2 size={24} className="animate-spin text-blue-600 mx-auto" /></div>
              ...
              <div key={plan.slug} className={`relative bg-white rounded-2xl border ${isPopular ? 'border-blue-200 shadow-2xl shadow-blue-500/10 scale-[1.02] lg:scale-105' : 'border-gray-100 shadow-sm'} p-8 card-hover`}>
                ...
                <ul className="space-y-3.5 mb-10">
                  {features.map((f: string, j: number) => (
                    <li key={j} className="flex items-start gap-3 text-sm text-gray-600"><Check size={16} className="text-green-500 mt-0.5 shrink-0" /><span>{f}</span></li>
                  ))}
                </ul>
                <Link href={plan.price_monthly > 0 ? '/dashboard/billing' : '/register'}
