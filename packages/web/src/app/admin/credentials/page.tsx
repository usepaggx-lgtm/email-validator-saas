...
export default function AdminCredentialsPage() {
  ...
  const LABELS: Record<string, string> = {
    ...
    consultas_access_token: 'Token de Acesso (Consultas)',
    consultas_token_id: 'Token ID (Consultas)',
  }
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            ...
            <select value={form.key} onChange={e => setForm({ ...form, key: e.target.value })} className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-purple-500/20">
              ...
              <option value="consultas_access_token">Token de Acesso (Consultas)</option>
              <option value="consultas_token_id">Token ID (Consultas)</option>
            </select>
          </div>
          <div>
